...
O3.density <- 
  ...
  labs(x = "Ozone AQI value", y= "Denisty", fill ="")
print(O3.density)
...
LTER.regression <- lm(EPAair$Ozone ~ EPAair$PM2.5)
print(LTER.regression)
summary(LTER.regression)
...
OzonebyAll.regression <-
  lm(data=EPAair.subset, PM2.5~ Ozone +Year + Month + mean_Lon +mean_Lat)
...
Litter.Totals.improved <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5, label = c("abc", "ab", "abcd", "bcd", "bcd", "a", "d", "abcd", "cd", "d", "abcd", "abcd"))) +
  ylim(0, 85)+
  arrange(Litter.Totals)
print(Litter.Totals.improved)
