...
huc8_sf3<- st_read('./Data/Spatial/NCHUC8.shp') %>%
  filter(SUBBASIN != 'Upper Neuse')

#Create a map
mapview(huc8_sf2, col.regions = 'purple') +
  mapview(huc8_sf3, col.regions = 'orange')
...
Wind_Energy <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) %>%
  ...
  group_by(HUC_8) %>%
  summarize(Sum_wind = sum(AvgWindEnergy))

#Join to HUC_8 features
huc8_join = merge(x = huc8_sf, 
                  )
