...
```{r}
#Apply the API key
census_api_key("d4ad7874c996c7a83fec0a81466309ddc07d8e22a", overwrite = TRUE, install = TRUE)
...
the_main_website <- read_html(the_website)

#2&3 Locate elements and read their text attributes into variables

the_states <- the_main_website %>% html_nodes("td:nth-child(1)") %>% html_text()
the_price <- the_main_website %>% html_nodes("td:nth-child(2)") %>% html_text()
the_capacity <- the_main_website %>% html_nodes("td:nth-child(3)") %>% html_text()
net_generation <- the_main_website %>% html_nodes("td:nth-child(4)") %>% html_text()
total_retail <- the_main_website %>% html_nodes("td:nth-child(5)") %>% html_text()
  
#3 Construct a dataframe from the values
energy_df <- data.frame("Name" = the_states,
                        "Avg Price" = the_price,
                        "MW" = the_capacity,
                        "MWh" = net_generation,
                        "Total Sales" = total_retail)
```
...
```{r create.scrape.function}
  ...
  #Fetch the website
the_url <- "https://www.eia.gov/electricity/state/"
the_main_website <- read_html(the_url)
...
=======
                    paste0('https://www.eia.gov/electricity/state/archive/',the_year,'/'))
>>>>>>> 5b53ddd2f9c6d2d3768b7de37109cf4203c032c9
...
#3 Construct a dataframe from the values
energy_df2 <- data.frame("Name" = the_states,
                        "Avg Price" = the_price,
                        "MW" = the_capacity,
                        "MWh" = net_generation,
                        "Total Sales" = total_retail)
