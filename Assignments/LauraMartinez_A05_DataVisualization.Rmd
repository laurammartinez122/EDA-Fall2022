...
read.csv("~/Documents/EDA-Fall2022/Data/Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv", stringsAsFactors = TRUE)

NIWO_MassTrap <- 

read.csv("~/Documents/EDA-Fall2022/Data/Processed/NEON_NIWO_Litter_mass_trap_Processed.csv",stringsAsFactors = TRUE)
...
plot_grid(Temp.boxplot, TP.boxplot, TN.boxplot, ncol = 1, align = 'h', 
          
rel_heights = c(2.5, 2.5, 4))
```

Question: What do you observe about the variables of interest over seasons 
and between lakes? 

> Answer: Both lakes follow a normal distribution for mean temperature, whereas mean tp_ug shows a more consistent trend across the months and seasons. Similarly, mean tn_ug follows a consistent pattern across months and seasons. One difference is that temperature does not show a lot of outlier data, where tp_ug and tn_ug have a lot of data skewing the IQR into a higher range. While the means are similar for Peter and Paul Lake, the tp_ug and tn_ug plots show that Paul Lake tends to have lower mean values across the year. 
